...
def deckAnalysis(df):
    ...
    df = pd.DataFrame({"code": cardIDList, "deckCount": cardDeckCount, "imgSource": cardImgSource, "count": cardAvgCount})
    df["% of decks"] = round(df["deckCount"] / totalDeckCount, 3)
    #df = df.drop(columns=["deckCount"])
    df["name"] = cardName
    

    df = df[["imgSource", "name", "% of decks", "count"]].sort_values(["% of decks", "count"], ascending=False).reset_index(drop=True)
    #print(df.to_string())
    return df
...
#figure out how to have the autogenerated tables display the card images 
